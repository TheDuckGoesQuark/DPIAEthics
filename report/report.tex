...
\hhline{-} %row no:2 
\multicolumn{1}{|p{\linewidth}|}{
    Figure \ref{fig:identify} shows an example of the facial recognition API usage flow for reference. 
% Collect
    Consumers of the cognitive services API provide all data used by the software, which includes images of people for identification and training. The URL of images are provided using HTTPS, which are then fetched by the service to be processed.
% Use
...
Data stored using a multi-tenant approach, where multiple customers data will be stored on the same hardware \cite{terrylanfear}. 
...
Access to the data and model requires a cryptographic key.

% Delete
The documentation shows that consumers of the API can delete anything they create (groups of people, individual people) with immediate effect \cite{documentation}. 
...
% Sharing
Data provided by customers is not shared with any third parties, and is not accessible by default to Microsoft. This is achieved through access control, network security, and encryption of data at rest.
% High Risk
...
Both correct and incorrect classifications could influence individuals being processed by the system. 
For example, a shop may use the facial recognition software to identify known criminals, which could result in customers of the shop being treated differently depending on the software output.
} \\ 
\hhline{-} 
\end{longtable} 
