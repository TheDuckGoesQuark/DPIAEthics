...
\begin{itemize}
    ...
    \item match data or combine datasets from different sources.
    \item collect personal data from a source other than the individual without providing them a privacy notice.
    \item profile children or target marketing or online services at them
\end{itemize}

Since the service does not monitor or restrict what data is provided and how it is collected, the last three cases can be incurred by users of the service. 
...
% Collect
Consumers of the cognitive services API (Application Program Interface) provide all data used by the software, which includes images of people for identification and training. The URL of images are provided using HTTPS (Hypertext Transfer Protocol Secure), which are then fetched by the service to be processed.
